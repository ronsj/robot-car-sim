...
function BoxObstacle({ rect, color }: { rect: Rect; color: string }) {
  ...
  return (
    <mesh position={[rect.x, obstacleHeight / 2, rect.y]} castShadow receiveShadow>
...
function Wall({ rect }: { rect: Rect }) {
  ...
  return (
    <mesh position={[rect.x, wallHeight / 2, rect.y]} castShadow receiveShadow>
...
function Robot({ robot }: { robot: RobotState }) {
  return (
    <group position={[robot.x, 0, robot.y]} rotation={[0, -robot.theta, 0]}>
      ...
      <mesh position={[0.35, 0.28, 0]} rotation={[0, 0, -Math.PI / 2]} castShadow>
...
export function Scene3D({ robot, world }: Scene3DProps) {
  ...
  return (
    <>
      ...
      <gridHelper args={[world.width, world.width, "#334155", "#1e293b"]} position={[0, 0.01, 0]} />

      {world.walls.map((wall, i) => (
        <Wall key={`wall-${i}`} rect={wall} />
      ))}
      {world.obstacles.map((obs, i) => (
        <BoxObstacle key={`obs-${i}`} rect={obs} color="#ea580c" />
      ))}
